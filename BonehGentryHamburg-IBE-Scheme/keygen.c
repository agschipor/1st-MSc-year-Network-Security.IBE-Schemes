...
//---------------------------------------------------------------------------------
void KeyGen(unsigned char* identity, mpz_t* rj, mpz_t n, mpz_t u, mpz_t p, mpz_t q, int l, char** error)
{
    int i, w;
    mpz_t uRj, z[4];
...
void WritePrivateParametersToFile(mpz_t *rj, int l, char** error)
{   
    int i;
...
int main(int argc, char *argv[])
{
    if (argc < 3) 
    {
        fprintf(stderr, "Usage: %s <identity> <l>\n", argv[0]);
        return 1;
    }
    ...
    rj = (mpz_t*)malloc(l * sizeof(mpz_t));
    ...
    KeyGen((unsigned char*)argv[1], rj, n, u, p, q, l, &error);
    ...
    WritePrivateParametersToFile(rj, l, &error);
